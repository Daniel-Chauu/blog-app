    ...
    username: {
      ...
      custom: {
        options: async (value: string) => {
          ...
          if (isExistUsername)
            throw new ErrorWithStatus(AUTH_MESSAGE.USERNAME_ALREADY_EXISTS, HTTP_STATUS_CODE.BAD_REQUEST)
        }
      }
    },
    email: {
      ...
      custom: {
        options: async (value: string) => {
          ...
          if (isExistEmail) throw new ErrorWithStatus(AUTH_MESSAGE.EMAIL_ALREADY_EXISTS, HTTP_STATUS_CODE.BAD_REQUEST)
        }
      }
    },
    password: {
